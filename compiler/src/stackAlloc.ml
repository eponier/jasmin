...
(* several printing functions are already defined in src/varalloc.ml *)
let pp_sao tbl fmt sao =
  ...
  let pp_sao_param fmt param =
    let pp_param_info fmt pi =
      ...
      Format.fprintf fmt "{ pp_ptr = %a; pp_writable = %b; pp_align = %s }" (Printer.pp_var ~debug:true) var pi.pp_writable (Prog.string_of_ws pi.pp_align)
    in
    match param with
  ...
  let pp_sao_alloc fmt alloc =
    ...
    let pp_pki fmt pki =
      ...
      | PIstkptr (v, z, x) ->
          ...
          Format.fprintf fmt "Stack ptr %a (of type %a) %a (pseudo-reg %a (of type %a))" (Printer.pp_var ~debug:true) v Printer.pp_ty v.v_ty pp_zone z (Printer.pp_var ~debug:true) x Printer.pp_ty x.v_ty
    in
    let (var, pki) = alloc in
    let var = Conv.var_of_cvar tbl var in
    Format.fprintf fmt "(%a, %a)" (Printer.pp_var ~debug:true) var pp_pki pki
  in
  let pp_sao_to_save fmt to_save =
    let (v, ofs) = to_save in
    let v = Conv.var_of_cvar tbl v in
    let ofs = Conv.bi_of_z ofs in
    Format.fprintf fmt "(%a, %a)" (Printer.pp_var ~debug:true) v Bigint.pp_print ofs
  in
  let pp_sao_rsp fmt rsp =
    ...
    | SavedStackReg v ->
      let v = Conv.var_of_cvar tbl v in
      Format.fprintf fmt "Reg %a" (Printer.pp_var ~debug:true) v
    | SavedStackStk ofs ->
      let ofs = Conv.bi_of_z ofs in
...
let pp_oracle tbl up fmt saos =
  ...
  let (_, fs) = p in
  Format.fprintf fmt "@[<v>Globals:@;<2 2>@[<v>%a@]@;Global_alloc:@;<2 2>@[<v>%a@]@;Stack_alloc:@;<2 2>@[<v>%a@]@]"
